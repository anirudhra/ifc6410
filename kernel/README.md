...
1. **CPUFreq Bootloader Pinning:**
   * Upstream mainline Linux 6.6 lacks the necessary Device Tree OPP tables, CPU clock wiring (`kpss-gcc`), and SAW dynamic regulator bindings for APQ8064 Krait cores.
   * `meta-ifc6410` disables `CONFIG_ARM_QCOM_CPUFREQ_NVMEM`, preventing driver probe errors (`-ENOENT`) and allowing all four Krait cores to run stably at the bootloader-initialized 1.7 GHz ceiling.
2. **Silencing Bogus Sensor Warnings:**
...
3. **Power Management & PM8921 Regulator Overrides:**
...
6. **802.11n enablement**
   * Overrides no HT bit in ATH6KL driver as the last release firmware does not advertise HT (802.11n) capability in firmware even though the system is capable. A driver is needed (and provided part of this package) to enable it.   
7. **Integrated Onboard Firmware Blobs:**
...
1. **CPU Clocks & Regulators:**
   ```bash
   dmesg | grep -iE 'qcom-cpufreq|ncp|pm8921'
   ```
   * *Expected Result:* No `-ENOENT` probe errors from `qcom-cpufreq-nvmem`. Regulators initialize without DT parsing failures for `pm8921_ncp`.
   * Check all 4 cores running:
     ```bash
     cat /proc/cpuinfo | grep processor
     ```

2. **Sensors / Clean Bootlog:**
   ```bash
   dmesg | grep -i bmp
   ```
   * *Expected Result:* Zero warnings regarding `SPI driver bmp280 has no spi_device_id`.

3. **Onboard Wi-Fi (Atheros AR6004 SDIO):**
   ```bash
   dmesg | grep -i ath6kl
   ```
   * *Expected Result:* Driver reports `ath6kl: ar6004 hw 3.0 sdio fw 3.5.0.349-1 api 5`, and interface `wlan0` appears in `ip link`.

4. **Onboard Bluetooth (Atheros AR3002 UART on GSBI6):**
   ```bash
   modprobe hci_uart
   ...
   ```
   * *Expected Result:* Device attaches cleanly and discovers nearby BLE/classic advertisements.
